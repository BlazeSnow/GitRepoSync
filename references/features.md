...
- 交互：双击或右键表格行后弹窗编辑；右键行弹出上下文菜单（“编辑 / 立即同步 / 删除”），连续右键其他行可直接切换菜单目标，点击菜单外或按 Esc 关闭；表格区（含表头/空白处）右键不弹出 WebView 原生菜单；**编辑弹窗左下角提供「删除仓库」按钮**（编辑模式显示，点击后弹出确认弹窗执行删除，与右键菜单的删除共用同一确认流程）
