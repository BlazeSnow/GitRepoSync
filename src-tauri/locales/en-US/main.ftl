...
log-sync-failed-reason = Failed to sync repository "{$name}": {$reason}
log-sync-stopped = Sync of repository "{$name}" was stopped
log-sync-stopped-cmd = Stopped sync
sync-no-source = Source URL (origin) is missing; cannot sync
sync-no-targets = No backup target remote configured; cannot sync
...
push-target-failed = Push to {$remote} failed: {$err}
warn-lfs-push = ⚠ LFS object pre-upload failed: {$err}
warn-lfs-config = ⚠ Failed to write LFS locking config, skipped: {$err}
