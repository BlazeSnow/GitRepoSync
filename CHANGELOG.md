...
- ⚡ 新增 `run.ps1` 快速启动脚本：默认构建后**后台启动** Debug 版（源码有变动时自动增量重建，脚本立即返回不占用终端），`-Dev` 进入热重载开发模式（启动前自动清理 1420 端口残留的开发服务器），`-Build` 打包发布版；自动切换终端 UTF-8 编码、检查依赖并在首次运行时安装前端依赖；每个阶段输出进度提示与耗时（修复源码变更检测中单文件递归枚举导致的 9 秒卡顿，无变更时秒级启动）
