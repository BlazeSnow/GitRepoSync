...
log-sync-failed-reason = 同步仓库「{$name}」失败：{$reason}
log-sync-stopped = 同步仓库「{$name}」已停止
log-sync-stopped-cmd = 停止同步
sync-no-source = 缺少源地址（origin），无法同步
sync-no-targets = 没有可用的备份目标远端，无法同步
step-clone = 从源仓库克隆到本地中转站
step-fetch = 拉取源仓库更新
step-lfs = 更新 LFS 文件
step-submodule = 更新 submodule
step-push = 推送到目标仓库
push-target-failed = 推送到 {$remote} 失败：{$err}
warn-lfs-push = ⚠ LFS 对象预上传失败：{$err}
warn-lfs-config = ⚠ LFS locking 配置写入失败，已跳过：{$err}
lfs-skipped = 未检测到 git-lfs，已跳过 LFS 更新
manually-stopped = 已手动停止
git-spawn-error = 无法启动 git：{$err}（请确认系统已安装 Git 并加入 PATH）
sync-unconfigured = 源地址或目标地址未配置
log-repo-discovered = 自动发现并登记仓库「{$name}」
