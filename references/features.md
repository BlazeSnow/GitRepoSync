...
- 下拉选框：选择一定时间内未同步的仓库进行同步（全部 / 1 / 3 / 7 / 30 天未同步，含从未同步的仓库）；**选择持久化到 localStorage**（`grs_stale_range`），切页后返回同步仓库页时保持上次选择；**范围筛选同时作用于表格**：选 N 天范围时表格仅显示该范围内已配置的仓库（与「开始同步」按钮的范围和计数一致，未配置仓库不显示），范围内无仓库时显示专属空态提示
...
- 表格列：仓库、源地址、目标地址、状态、上次同步时间；**「仓库」「状态」「上次同步」表头均可点击排序**：三态循环（升序 ↑ → 降序 ↓ → 恢复默认），状态升序按问题优先（失败 > 同步中 > 已停止 > 未同步 > 成功），时间升序「从未同步」最先、按时间从旧到新，名称序与后端默认一致；未配置仓库固定排最后，不随方向反转
- 交互：双击或右键表格行后弹窗编辑；右键行弹出上下文菜单（“编辑 / 立即同步 / 删除”），连续右键其他行可直接切换菜单目标，点击菜单外或按 Esc 关闭；表格区（含表头/空白处）右键不弹出 WebView 原生菜单；**悬停表格行显示结构化摘要**（整体状态与上次同步时间 + 每个备份目标的状态与失败/警告详情）；**编辑弹窗左下角提供「删除仓库」按钮**（编辑模式显示，点击后弹出确认弹窗执行删除，与右键菜单的删除共用同一确认流程）
...
- **同步失败记录原因**：拉取/克隆阶段的 git 错误直接作为原因；多目标推送时按目标汇总错误（「推送到 {remote} 失败：{错误详情}」），日志条目形如「同步仓库「demo」失败：<原因>」；原因折叠空白并按字符截断（300 字符上限）
