...
- 🏢 提供商页面：支持 GitHub、GitLab，填入对应平台的 Personal Access Token（PAT）即可列出账户及其组织；PAT 入库，界面只显示掩码
...
- 🌍 多语言支持（简体中文 / English）：后端采用 fluent-i18n，全部文案集中在 Fluent 语言包（`locales/*.ftl`），缺失翻译自动回落中文；前端为轻量词典式 i18n，登录页与侧边栏可切换语言（首次启动按系统语言选择，手动切换后记忆）；MCP 工具描述、参数描述与运行时消息按 Agent 客户端 initialize 的 `locale` 返回，可用环境变量 `GIT_REPO_SYNC_LANG`（`zh` / `en`）强制指定；工具名称为协议契约不随语言变化
